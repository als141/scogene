...
% ===== List spacing — very tight =====
\setlist{nosep,leftmargin=1.2em,topsep=0pt,partopsep=0pt,parsep=0pt,itemsep=0pt}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.7\textwidth]{docs-cycle.png}
...
\end{tikzpicture}
\caption{知識グラフの例：空間図形（習得12\%）でつまずいた生徒の根本原因を因数分解（38\%）まで遡り、段階的に習得させる。}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\columnwidth]{docs-robocon.jpg}
  \caption{NHK高専ロボコン2021関東甲信越地区大会}
\end{figure}

ETSUZAN採択 無限問題生成AI「モンジェネ」チームリーダー（\url{https://youtu.be/PCIbYSYGWrI}、\url{https://meikan.etsuzan.org/}）

\begin{itemize}
  \item \textbf{塾講師アルバイト（5年目）}：新潟県長岡市の個別指導塾で高専4年次から務めている。右も左もわからない新人先生時代から始まり、数々の受験期を乗り越え、現在は塾講師80名を束ねるリーダーとして、生徒の将来の選択肢の広がりを提供している。
  \item \textbf{学園祭実行委員長}：長岡技術科学大学学園祭「技大祭」の実行委員会に所属。学部4年次、第43回技大祭実行委員長を務めた。1年間、約200名の組織のトップとして、責任ある立場を全うした。（広報誌掲載 \url{https://www.nagaokaut.ac.jp/outline/pr/assets/VOS231web.pdf}）
  \item \textbf{NHK高専ロボコン}：高専1年から4年まで出場。4年次に、「超絶技巧」がテーマの大会で、前人未踏な「ターザンロボット」を開発。チームリーダー兼部長。地区大会で本田技研工業株式会社特別賞を受賞。（\url{https://www.nagaoka-ct.ac.jp/58563/}、\url{https://youtu.be/S-8Iz0e02Xk}）
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\columnwidth]{docs-nutmeg.png}
  \caption{NUTMEG（技大祭実行委員会 情報局）メンバー}
\end{figure}

\textbf{【開発実績】}
\begin{itemize}
  \item \textbf{NUTMEG}（学園祭実行委員会 開発部門）：Next.js/Railsを用いた複数アプリケーションの開発。開発団体の代表。（\url{https://blog.nutmeg.cloud/}）
  \item \textbf{長岡インターンシップサイト}（業務委託）：要件定義から実装・デプロイまでを1.5ヶ月で完遂。
  \item \textbf{モンジェネ}（経産省AKATSUKI事業）：LLMで数学入試対策問題を無限に生成するアプリを開発。（\url{https://youtu.be/PCIbYSYGWrI}、\url{https://meikan.etsuzan.org/}）
\end{itemize}

\textbf{【研究実績】}
\begin{itemize}
  \item 修士研究テーマ：「GraphVAEを用いたSlackチャットログに基づく組織構造再構成に関する研究」（長岡技科大 機械学習理論研究室）。深層生成モデルで組織のコミュニケーション構造を可視化・再構成する研究。
\end{itemize}

\subsection*{小武 壮太}

グラフィックデザインを主軸に、イベント広報物制作や空間演出のディレクションを行ってきた。現在はFigmaでのUI/UX設計や、生成AIを活用したフロントエンドプロトタイピングを実践している。

\begin{itemize}
  \item \textbf{モンジェネのUI設計}（ETSUZAN採択）：講師の認知負荷と操作コストを下げるUI設計。生成AIを活用しフロントエンドコードを出力・検証。（\url{https://youtu.be/PCIbYSYGWrI}、\url{https://meikan.etsuzan.org/}）
  \item \textbf{NUTMEG}での開発：学内団体管理システム「GM2」のフロントエンド担当（Vue.js, TypeScript）。UI実装、CSV/PDF出力機能追加、権限管理修正等。（\url{https://blog.nutmeg.cloud/}）
\end{itemize}
...
\textbf{【開発実績】}
\begin{itemize}
  \item \textbf{マーケティングAIエージェントシステム}（株式会社b\&q）：100以上のツールを統合したMulti-Agent Architecture設計・実装。
  \item \textbf{SEO記事生成SaaS}（株式会社新大陸 テックリード）：LLM・エージェントSDK活用の記事自動生成システム。月間数千記事の生成実績。
  \item \textbf{SakeScope}（経産省AKATSUKI事業）：マルチモーダルLLMによるAI日本酒ソムリエ。（\url{https://youtu.be/noTHRUNfiwA}、\url{https://meikan.etsuzan.org/}）
\end{itemize}

\textbf{【研究実績】}
\begin{itemize}
  \item 修士研究テーマ：「進化的群知能に基づくLLMエージェント集団の協調最適化」（新潟大 計算知能研究室）。複数LLMエージェントを進化的アルゴリズムで最適化する研究。
\end{itemize}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\columnwidth,height=5cm,keepaspectratio]{docs-hobby-takahashi.png}
  \caption{地域ナイター杯で準優勝}
\end{figure}

趣味は野球である。小学校3年生から始め、大学院修士1年となった現在も軟式野球部に所属し、精力的に活動している。昨年は、約40チームが出場する地域のナイター杯において、チーム一丸となり準優勝を果たした。プレー面では、体格を活かしたバッティングを持ち味とし、5番レフトとしてチームの勝利に貢献している。もともと体力がなく持久走が大の苦手だったが、鬼監督の厳しい練習を通じて、粘り強い体力と困難に立ち向かう気合を培った。現在は野球に加え、バスケットボールやバレーボールにも挑戦するなど、スポーツ全般を楽しんでいる。

\subsection*{若月 耕紀}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\columnwidth]{docs-hobby-wakatsuki.png}
  \caption{実家の犬}
\end{figure}

趣味は動物鑑賞である。実家が動物病院を経営しており、生まれた時から動物に囲まれて育った。当時は、犬が9匹、猫が6匹、インコが2羽、うさぎが1羽いた。現在は一人暮らしで部屋に動物がいない生活を送っており、その反動でペットショップや猫カフェに行くことが増えた。言葉の通じない生き物と向き合う経験が、相手の裏側を想像し引き出すマネジメントスキルに活きている。

\subsection*{小武 壮太}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\columnwidth,height=5cm,keepaspectratio]{docs-hobby-kotake.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.55\columnwidth,height=5cm,keepaspectratio]{docs-hobby-masuda.jpg}
  \caption{高校バレーボール部時代}
\end{figure}

趣味はバレーボールである。小・中・高と部活動を続け、JOCで秋田県選抜になった経験がある。現在も友人やサークルなどでプレーを続けており、競技としての楽しさはもちろん、チームで連携して結果を出す面白さを大切にしている。

もう一つの趣味は映像制作である。最初は純粋に作ること自体が楽しく、構成・撮影・編集を試しながら学んでいたが、気づけば様々な場面で自然と活用するようになっていた。バレー部の紹介動画やイベントのダイジェストなどを、自ら企画し、撮影し、編集して形にしてきた。「何を、どの順序で、どう見せれば伝わるか」を考えながら制作することは、対象の魅力を引き出し、伝えたい内容を届く形に整える力につながっていると感じている。
...
私は塾講師として5年間、生徒と向き合ってきた。その中で確信したのは、教育の本質は「人と人との関わり」にあるということだ。AIがどれだけ進化しても、生徒の目を見て「頑張ったね」と声をかけること、進路に悩む生徒の話を聞くこと、そうした人間的な関わりこそが生徒を成長させる。モンジェネは、その関わりの時間を最大化するためのツールであり、教育の未来を人間の手に取り戻すプロジェクトである。
