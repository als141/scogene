...
\titlespacing*{\subsubsection}{0pt}{0.15em}{0.04em}
...
塾講師の業務は「教えること」である。しかし、解き直しの場面においては、生徒の要望にあった問題を探すという業務が発生する。塾に置いてある大量の紙の問題集から、生徒の要望にあった問題を探し出す作業は5分〜10分ほどかかってしまう。1人の生徒に割ける時間は実質\textbf{40分}。この40分のうち、問題集から適切な問題を探す作業に\textbf{約10分}---図\ref{fig:time_pie}に示すように、指導時間の25\%が「紙をめくる時間」に消えてしまう。さらには、やっと見つけた問題が生徒の苦手にマッチしているかはわからない。

\begin{figure}[t]
\centering
\begin{tikzpicture}
  % 75% = 270 degrees (from 90 to 360), 25% = 90 degrees (from 0 to 90)
  \fill[mainblue!60] (0,0) -- (90:1.3) arc (90:360:1.3) -- cycle;
  \fill[dangerred!60] (0,0) -- (0:1.3) arc (0:90:1.3) -- cycle;
  \node[font=\scriptsize\sffamily\bfseries, text=white] at (225:0.8) {指導};
  \node[font=\tiny\sffamily, text=white] at (225:0.45) {30分(75\%)};
  \node[font=\scriptsize\sffamily\bfseries, text=white] at (45:0.75) {問題探し};
  \node[font=\tiny\sffamily, text=white] at (45:0.4) {10分(25\%)};
\end{tikzpicture}
\caption{1コマ40分の時間配分：指導時間の4分の1が問題探しに消費されている}
\label{fig:time_pie}
\end{figure}
...
LLMで直接生成するのではなくPythonコードを生成・外部実行する方式を採用し、数値計算精度と図形描画精度を担保した。対象は新潟県公立高校入試の数学（中学生用）に限定した。
...
近年、AIを教育に応用する研究が世界的に加速している。小学校から高校までを対象としたAI個別指導システムの大規模レビューでは、従来指導に対して中〜大の効果量で学習効果の向上が確認されている\,[7]。また、AIによる数学の解答エラーの自動分類が専門家と同等の精度に到達しつつあり\,[8]、2025年にはVision言語モデルによる手書き数学答案の自動採点\,[9]、2026年には知識グラフとAIを組み合わせた個別最適化問題生成\,[10]や適応的な問題推薦手法\,[11]が実証されるなど、本提案の技術的基盤となる知見が急速に蓄積されている。
...
ETSUZAN期間中のモンジェネを教育関係者に話をしたところ、高校教員や大学教員の業務負担軽減にもつながるのではと感じた。作問と採点に追われる教員の厳しい労働環境は、教育の質低下を招いている。

今回提案するソフトウェアは、採点結果・生徒データを活かした問題生成により、塾講師の問題用意の負担軽減と生徒の学習効率向上を目的とする。この未踏期間では、まずは塾講師の私自身が使いやすい「問題作成・採点ソフト」を目指す。

\subsection{提案システム}

本システムは、塾講師の通常の授業の流れを大きく妨げないように設計する。図\ref{fig:cycle}のように、\textbf{「採点・苦手分析」→「教師へのフィードバック＋対策問題生成」→「生徒が解く」}のサイクルを繰り返すことで、\textbf{塾講師の問題探し・採点にかかる時間を大幅に削減}し、生徒と向き合う指導時間を最大化する。
...
本システムは、前述の2つの柱を実現するために、大きく3つのフェーズで動作する。

\begin{enumerate}[leftmargin=2em]
  \item \textbf{採点・分析}：生徒の手書き答案をAIが読み取り、途中式レベルで自動採点し、エラーの種類を分類する。
  \item \textbf{フィードバック・学習管理}：蓄積された採点データから生徒ごとの苦手を分析し、塾講師に指導方針を提案する。知識グラフ上で概念ごとの習得度を追跡する。
  \item \textbf{問題生成}：苦手プロファイルに基づき、苦手を克服するための問題をDB検索またはAI生成で提供する。
\end{enumerate}

図\ref{fig:system_overview}に各コンポーネントの配置を示す。以下、それぞれの\textbf{目的}と\textbf{実現手法}を述べる。

\begin{itemize}
  \item \textbf{採点Agent}：手書き答案を自動採点し、間違いの種類を特定する。GPT-5.4のVision機能で答案画像を読み取り\,[9]、OpenAI Agents SDKで途中式レベルの採点を行う。Code Interpreter上でPython（PIL）を実行し、答案画像に直接赤ペンで添削マークを書き込む。間違いを4種類（読解・変換・処理・書き写し）に自動分類し、苦手の本質を特定する。

  \item \textbf{問題生成Agent}：苦手に特化した数学問題を無限に生成する。LLM（GPT-5.4 / Claude）にCode Interpreter（Python実行環境）を組み合わせ、計算過程はSymPyで自動検証し、図形問題はMatplotlibで描画する。ETSUZAN期間中に開発した3パターン生成を基盤に、苦手プロファイルに基づく個別最適化に拡張する。

  \item \textbf{知識グラフ}：数学の概念間の前提関係を管理する。各ノードは「因数分解」「三平方の定理」などの\textbf{数学単元}を表し、各エッジは「AはBの前提知識である」という\textbf{前提関係}を表す有向非巡回グラフ（DAG）である。学習指導要領と塾講師の知見をもとに構築し、つまずきの根本原因を自動特定する\,[10]。

  \item \textbf{適応学習エンジン}：生徒が各概念をどの程度理解しているかをリアルタイムに推定する。手法としてBayesian Knowledge Tracing（BKT）\,[3]を採用する。BKTは正答・誤答の系列から概念ごとの\textbf{習得確率}を推定する確率モデルであり、教育工学分野で30年以上の実績がある。「この概念の習得確率は65\%」のように\textbf{塾講師にとって直感的に解釈可能}な点が採用理由である。FSRS v6で最適な復習タイミングを決定し、忘却を防ぐ。

  \item \textbf{問題データベース}：蓄積された問題の難易度を客観的に評価し、最適な問題を検索する。統一模試の正答率データからIRT（項目反応理論）パラメータを較正する。新規問題の難易度推定にはLightGBMを用い、問題文の特徴量（単元タグ・必要公式数・計算ステップ数等）から正答率を予測する。埋め込みベクトル（768次元）によるセマンティック検索と組み合わせ、Supabase上に構築する。
...
採点結果は蓄積型の\textbf{苦手プロファイル}として記録する。概念ごとの習得確率をBKT\,[3]で追跡する。従来のBKTは、初めて利用する生徒や新規追加した問題に対して解答履歴がないため習得度を推定できない（コールドスタート問題）。本システムではLLMが問題文の内容を解析して初期パラメータを推定することで、この問題を解決する\,[4]。Bloomの完全習得学習で標準的に用いられる閾値（80--90\%）\,[1]に基づき、習得確率が\textbf{85\%}を超えた概念を「習得済み」と判定する。
...
\begin{itemize}
  ...
  \item \textbf{具体的な指導アクション}---知識グラフの前提関係と採点データから「空間図形は展開図から復習が必要」等の指導提案をLLMが生成
\end{itemize}
...
ETSUZAN期間中の3パターン生成を基盤とし、苦手プロファイルに基づく3つのモードに拡張する。
...
個別最適化問題の生成では、「元の問題文」「模範解答」「生徒の手書き答案」「採点フィードバック」の\textbf{4点をLLMに入力}し、エラー分類に応じた苦手克服問題を生成する。問題の提供は\textbf{DB検索とLLM新規生成のハイブリッド}で行い、Thompson Sampling\,[11]により探索と活用のバランスを最適化する。具体的には、各問題の「学習効果（出題後に類似問題の正答率が向上したか）」を報酬としてベイズ更新し、DB既存問題の活用とLLM新規生成の探索を確率的に切り替える。生成問題はPythonコード実行による数学的検証を経てから出題する。
...
\textbf{習得判定の3条件:}
\begin{enumerate}[leftmargin=2em]
  \item 概念ごとの習得確率（BKT）が\textbf{85\%以上}（Bloomの完全習得学習の標準閾値\,[1]）
...
データ層 & 内容と意義 \\
\midrule
第1層：元データ & 統一模試協会の大規模問題データ（正答率付き）、各県入試問題。提携なしには入手不可能 \\
...
DBに蓄積された問題は埋め込みベクトルによるセマンティック検索に対応し、「この生徒の苦手に最も適した問題」を高速に発見する。後発の競合がシステムを模倣しても、このデータ蓄積は即座に再現できない。生徒が問題を解く→回答データが蓄積→推薦精度が向上→学習効果が上がる→ユーザーが増える、という\textbf{フライホイール効果}で、時間が経つほどシステムの価値が加速度的に高まる。
...
\begin{itemize}
  \item \textbf{生徒の学力向上}：AI個別指導システム（ITS: Intelligent Tutoring System）の大規模レビュー\,[7]では、従来の教師主導指導に対し中〜大の効果量で優位性が確認されている。生成AIチューターは対面指導の2倍の学習効率を数百分の一のコストで達成した事例もある\,[2]。本システムの学習効果を実証実験で検証する。
...
なお、LLMを用いた教育システムに関するサーベイ\,[6]および2025--2026年の最新研究\,[9,\,10,\,11,\,12]を精査した上で、「知識グラフによる根本原因分析」と「LLMによる問題の動的生成」を\textbf{紙の手書き答案}に対して統合的に適用するシステムは、国内外の学術研究・商用サービスを含めて確認されていない。
...
AI/ML & OpenAI API (GPT-5.4), Claude API, Bayesian Knowledge Tracing, FSRS v6 \\
...
\begin{enumerate}[leftmargin=2em,label={[\arabic*]}]
  ...
  \item Nath, S.\,et~al. (2025). Can Vision-Language Models Evaluate Handwritten Math? \textit{Proc.\ ACL 2025}, 14784--14814.
  \item Wan, H.\,et~al. (2026). Beyond Static Question Banks: Dynamic Knowledge Expansion via LLM-Automated Graph Construction and Adaptive Generation. \textit{arXiv:2602.00020}.
  \item De~Kerpel, F.\,et~al. (2026). A Bandit-Based Approach to Educational Recommender Systems: Contextual Thompson Sampling for Learner Skill Gain Optimization. \textit{arXiv:2602.04347}.
  \item Li, Z.\,et~al. (2026). Thinking As a Teacher: LLM Supporting Knowledge Tracing Leveraging Knowledge Graphs. \textit{Information Fusion}, 126(A), 103577.
\end{enumerate}
\end{small}
