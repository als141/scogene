...
\titleformat{\section}
  ...
  {\thesection}{0.8em}{}
  [\vspace{-0.5em}\textcolor{mainblue}{\rule{\linewidth}{1.5pt}}]
...
\vspace{0.3em}
\begin{center}
\usebox{\mongenecompare}\par\vspace{2pt}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.95\textwidth]{system-arch-new.jpg}
...
\begin{tikzpicture}[
  ...
  mastered/.style={rectangle, rounded corners=4pt, draw=successgreen, fill=successgreen!10,
    font=\small\sffamily, minimum height=1.0cm, minimum width=1.8cm, align=center, line width=1pt},
...
氏名 & 所属 & 担当 \\
\midrule
高橋創真 & \parbox[t]{3.5cm}{長岡技術科学大学 大学院工学研究科\\工学専攻 情報・経営システム工学分野} & チームマネジメント、ユーザヒアリング、要件定義 \\[6pt]
若月耕紀 & \parbox[t]{3.5cm}{長岡技術科学大学 大学院工学研究科\\工学専攻 情報・経営システム工学分野} & フロントエンド・バックエンド実装全般 \\[6pt]
小武壮太 & \parbox[t]{3.5cm}{長岡技術科学大学 大学院工学研究科\\工学専攻 情報・経営システム工学分野} & UI/UXデザイン、システムデザイン \\[6pt]
舛田　岳 & \parbox[t]{3.5cm}{新潟大学 大学院自然科学研究科\\電気情報工学専攻 情報社会デザイン科学コース 博士前期課程1年} & LLM・ML・インフラ/システム開発全般 \\
\bottomrule
\end{tabularx}
\end{footnotesize}
...
\begin{itemize}
  \item \textbf{塾講師5年目}：新潟県長岡市の個別指導塾で高専4年次から勤務。右も左もわからない新人先生時代から始まり、数々の受験期を乗り越え、現在は講師80名を束ねるリーダーとして生徒の将来の選択肢を広げている。本提案のドメイン知識の源泉。
  \item \textbf{NHK高専ロボコン}：高専1--4年次に出場。4年次にチームリーダー兼部長として前人未踏な「ターザンロボット」を開発し、本田技研工業特別賞受賞。1年次は部品加工、2年次はロボット制御、3年次はチームリーダーと段階的にモノづくりを経験。

  高専ロボコン2021アーカイブ \url{https://youtu.be/S-8Iz0e02Xk}
  \item \textbf{学園祭実行委員長}：第43回技大祭の実行委員長。約200名の組織を1年間統括。長岡技大広報誌掲載（\url{https://www.nagaokaut.ac.jp/outline/pr/assets/VOS231web.pdf}）。
\end{itemize}

\subsection*{若月 耕紀}

\begin{itemize}
  \item \textbf{NUTMEG代表}：技大祭実行委員会の開発部門でNext.js・Railsを用いた複数アプリケーション（シフト管理・出展団体管理・学習コンテンツ管理）を開発・統括。NUTMEG公式Blog：\url{https://blog.nutmeg.cloud/}
  \item \textbf{長岡インターンシップサイト}（業務委託）：Web制作会社からの業務委託。要件定義から実装、デプロイまでを1.5ヶ月で完遂。
  \item \textbf{モンジェネ開発}：ETSUZANにてLLMによる問題生成エンジンのフルスタック実装を担当。数値計算プログラムや図形描画プログラムの生成で信頼性の高い問題を実現。
  \item \textbf{修士研究}：「GraphVAEを用いたSlackチャットログに基づく組織構造再構成」（機械学習理論研究室）。深層生成モデルで組織内コミュニケーション構造を可視化・再構成する研究。
\end{itemize}

\subsection*{小武 壮太}

グラフィックデザインを主軸に、イベントの広報物制作や空間演出のディレクションを行ってきた。現在はその知見をWeb領域へ拡張し、Figmaを用いたUI/UX設計や生成AIを活用したフロントエンドのプロトタイピングを行っている。

\begin{itemize}
  \item \textbf{モンジェネUI設計}（ETSUZAN）：講師の認知負荷と操作コストを下げるため、検索以外のフローをクリック操作のみで完結させるUIを設計。生成AIでフロントエンドコードを出力・検証し、エンジニアへのイメージ共有を円滑にして開発効率を向上。
  \item \textbf{NUTMEGフロントエンド開発}：学内団体管理システム「GM2」をVue.js/TypeScriptで実装。FigmaデザインのUI実装に加え、CSV/PDF出力機能の追加や権限管理の修正を担当。NUTMEG公式Blog：\url{https://blog.nutmeg.cloud/}
\end{itemize}

\subsection*{舛田 岳}

\begin{itemize}
  \item \textbf{マーケティングAIエージェント}（株式会社b\&q）：100以上のツール（GWS, GA4, Ahrefs, GSC, Meta Ads等）を統合したMulti-Agent Architecture設計・実装。
  \item \textbf{SEO記事生成SaaS}（株式会社新大陸テックリード）：LLM・エージェントSDK活用。月間数千記事の生成実績。
  \item \textbf{SakeScope}（ETSUZAN）：マルチモーダルLLMによるAI日本酒ソムリエ。テレビ放映（\url{https://youtu.be/LLsKAEKmDJ4}）。
  \item \textbf{修士研究}：「進化的群知能に基づくLLMエージェント集団の協調最適化」（新潟大学 計算知能研究室）。複数のLLMエージェントを進化的アルゴリズムで最適化する研究。
\end{itemize}

% ============================================================
\section{プロジェクト遂行にあたっての特記事項}

高橋・若月・小武は長岡技術科学大学大学院 情報・経営システム工学分野 修士1年、舛田は新潟大学大学院 自然科学研究科 修士1年に所属し、来年度修士2年に進級する。全員が大学院の授業単位を取得済みで、修士研究のみを抱える状態である。指導教員との関係も良好で、未踏応募について容認されている。

開発場所は長岡技大 機械学習理論研究室と新潟大 計算知能研究室を使用する。高橋は塾に週複数日出勤しており、開発と実証実験のサイクルを高速に回せる。

\noindent\textbf{チームの経緯：}高橋・若月・小武は長岡技大の同期であり、学園祭実行委員会（NUTMEG）での開発活動を通じて信頼関係を構築した。舛田は新潟大の院生で、ETSUZAN同期採択者として出会い、互いのプロジェクト（モンジェネ・SakeScope）で協力関係を深めた。全員がETSUZAN修了生である。
...
\begin{wrapfigure}{r}{0.40\columnwidth}
  \vspace{-1em}
  \centering
  \includegraphics[width=0.38\columnwidth]{docs-hobby-takahashi.png}
  {\scriptsize\sffamily 地域ナイター杯で準優勝}
  \vspace{-1em}
\end{wrapfigure}

趣味は野球である。小学校3年生から始め、大学院修士1年の現在も軟式野球部に所属し精力的に活動している。昨年は約40チームが出場する地域のナイター杯で、チーム一丸となり準優勝を果たした。体格を活かしたバッティングを持ち味とし、5番レフトとしてチームの勝利に貢献している。もともと体力がなく持久走が大の苦手だったが、鬼監督の厳しい練習を通じて粘り強い体力と困難に立ち向かう気合を培った。現在は野球に加え、バスケットボールやバレーボールにも挑戦するなど、スポーツ全般を楽しんでいる。今後もスポーツを通じて、心身のタフさを磨き続けたい。

\subsection*{若月 耕紀}

趣味は動物鑑賞である。実家が動物病院を経営しており、生まれた時から動物に囲まれて育った。当時は犬が9匹、猫が6匹、インコが2羽、うさぎが1羽いた。現在は一人暮らしをしているため、部屋に動物がいない生活を送っている。その反動か、ペットショップや猫カフェなどに行くことが増えた。言葉の通じない生き物を相手にすることを生まれた時から行っていたことが、今のマネジメントスキルにも活きている。相手が言葉にすることのできない裏側を想像し、引き出す能力につながっていると言える。
...
\begin{wrapfigure}{r}{0.38\columnwidth}
  \vspace{-1em}
  \centering
  \includegraphics[width=0.36\columnwidth]{docs-hobby-kotake.png}
  {\scriptsize\sffamily 技大祭の広報デザイン}
  \vspace{-1em}
\end{wrapfigure}

趣味はデザイン制作。技大祭実行委員の広報としてInstagramの画像制作・運用を担当。依頼者へのヒアリングを通じて企画の意図を深く汲み取り、受け手がどう感じるか想像力を巡らせて形にすることにこだわる。ぜひ公式アカウント（\url{https://www.instagram.com/nutfes}）をご覧いただきたい。この「相手の想いを翻訳する力」がUI/UX設計の根幹を支えている。

\subsection*{舛田 岳}

趣味はバレーボールである。小・中・高と部活動を続け、JOCで秋田県選抜になった経験がある。現在も友人やサークルなどでプレーを続けており、競技としての楽しさはもちろん、チームで連携して結果を出す面白さを大切にしている。

もう一つの趣味は映像制作である。最初は純粋に作ること自体が楽しく、構成・撮影・編集を試しながら学んでいたが、気づけば様々な場面で自然と活用するようになっていた。バレー部の紹介動画やイベントのダイジェストなどを自ら企画・撮影・編集して形にしてきた。「何を、どの順序で、どう見せれば伝わるか」を考えながら制作する過程は、対象の魅力を引き出し、伝えたい内容を届く形に整える力につながっている。

% ============================================================
\section{将来のITについて思うこと・期すること}

AIの発展とその応用で様々な作業が効率化されており、省人化・無人化になった業務も多いだろう。人間の学習作業「教育」でもその利用は進んでいるが、教育と人は切り離すことはできない。

誰しも学生のころ、「この先生の話はちゃんと聞こうかな」なんて考えたことがあると思う。この現象は先生との関係性によって生まれるものである。英語学習アプリや常時質問可能なAIチャットがあれば「もはや先生はいらないのでは？」と考えてしまうかもしれない。

しかし、生徒の「モチベーション管理」はIT/AIが踏み込めない領域と考える。感情、非合理的な判断、人間関係は機械的なものでは構築・理解はできない。AIは生徒と対話することはできても、真にコミュニケーションを取っているとは言えない。さらには、憧れ・夢を抱かせることもできないと考える。
...
私は塾講師として5年間、生徒と向き合ってきた。その経験から確信しているのは、「教育の質は、先生と生徒が向き合う時間の量に比例する」ということだ。モンジェネで生まれる時間が、1人でも多くの生徒の未来を変えることを、心から願っている。
