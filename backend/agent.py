...
| 要素 | 色 (RGBA) | 用途 |
|------|-----------|------|
| 正解チェック ✓ | (0, 160, 0, 200) | draw.line() で2本線のチェックマーク |
| 不正解バツ ✗ | (220, 30, 30, 200) | draw.line() で2本線のバツマーク |
| エラー囲み ○ | (220, 30, 30, 180) | draw.ellipse() で誤答箇所を囲む |
| 注意波線 〜 | (255, 200, 0, 150) | sin波の連続線で注意箇所に下線 |
| コメント文字 | (220, 30, 30, 200) | 白背景 (255,255,255,200) 付きテキスト |
| 得点表示 | (200, 0, 0, 240) | 右上に大きく表示、白枠付き |

### 描画の具体的な手順
1. 画像のビジョン分析で各問題の解答位置を特定する
2. 正解の問題 → 解答の横に緑色チェックマーク
3. 不正解の問題 → 解答の横に赤色バツマーク、誤答部分を赤丸で囲む
4. 部分点の問題 → 正しい部分にチェック、間違った部分にバツ
5. テキストコメント → 白背景付きで誤りの指摘や正解を記述（英数字・記号で）
6. 合計得点 → 画像右上に大きく「8/10」のように表示
7. 全解答画像に対して処理を繰り返す
